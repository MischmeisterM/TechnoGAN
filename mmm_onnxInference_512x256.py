...
# for model trained on 512x256 px spectrograms
...
GENERATORSAVELOCATION = "data/generator_512x256_v2.onnx"
...
def generate(seed):
    ...
    r = sess.run(["conv2d_transpose_4"], {"x": input1})[0]
    return r
...
def generate_single_wave(seed, save_img=False):
    ...
    destwave, wflags = wtb.powergram_to_wave(im=im, hops=4, spectpow=2)
...
def generate_fade(startseed, endseed, steps):
    ...
    for i in range(steps):
        ...
        destwavestep, wflags = wtb.powergram_to_wave(im=im, hops=4, spectpow=2)
