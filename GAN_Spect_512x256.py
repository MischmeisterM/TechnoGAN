# training tf model for 512x256 pixel spectrograms
# for more detailed comments refer to GAN_Spect_128x256.py
...
PROJECTDIR = "D:/work/2022/TechnoGAN/networks/Spect_512x256_v2/"
...
BUFFER_SIZE = len(list(pathlib.Path(PROJECTDIR + "dataset/").glob('*.png')))
print(f'Loading dataset... {BUFFER_SIZE} pngs')
BATCH_SIZE = 10
noise_dim = 64
...
def make_generator_model():
    ...
    model.add(layers.Dense(32 * 64 * 128, use_bias=False, input_shape=(noise_dim,)))
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())
    print(model.output_shape)

    model.add(layers.Reshape((32, 64, 128)))
    # 32 x 64
    print(model.output_shape)

    model.add(layers.Conv2DTranspose(128, FILTER5, strides=(1, 1), padding='same', use_bias=False))
    # 32 x 64
    print(model.output_shape)
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())

    model.add(layers.Conv2DTranspose(96, FILTER5, strides=(2, 2), padding='same', use_bias=False))
    # 64 x 128
    print(model.output_shape)
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())
...
def make_discriminator_model():
    ...
    model = tf.keras.Sequential()
    model.add(layers.Conv2D(64, FILTER5, strides=(2, 2), padding='same', input_shape=[256, 512, 1]))
    model.add(layers.LeakyReLU())
    ...
    model.add(layers.Conv2D(160, FILTER5, strides=(2, 2), padding='same'))
    model.add(layers.LeakyReLU())
    model.add(layers.Dropout(0.3))
    print(model.output_shape)
